<template>
  <div :id="popperId" ref="popover" class="v-popper__popper" :class="[
    themeClass,
    classes.popperClass,
    {
  ...
  } : undefined" :aria-hidden="shown || autoHide ? 'false' : 'true'" :tabindex="autoHide ? 0 : undefined"
    :data-popper-placement="result ? result.placement : undefined" @keyup.esc="autoHide && $emit('hide')">
    <div class="v-popper__backdrop" @click="autoHide && $emit('hide')" />
    <div class="v-popper__wrapper" :style="result ? {
      ...
      <div ref="inner" class="v-popper__inner">
        <template v-if="mounted">
          ...
          <ResizeObserver v-if="handleResize" @notify="$emit('resize', $event)" />
        </template>
      </div>

      <div ref="arrow" class="v-popper__arrow-container" :style="result ? {
