<template>
  <Popper ref="popper" v-slot="{
    ...
    <PopperContent ref="popperContent" :popper-id="popperId" :theme="finalTheme" :shown="isShown"
      :mounted="shouldMountContent" :skip-transition="skipTransition" :auto-hide="autoHide"
      :handle-resize="handleResize" :classes="classes" :result="result" @hide="hide" @resize="onResize">
...
export default defineComponent({
  ...
  methods: {
    getTargetNodes() {
      const popperContent = this.$refs.popperContent as any
      return Array.from(this.$el.children)
        .filter(node => node !== popperContent.$el)
    },
  },
})
</script>
