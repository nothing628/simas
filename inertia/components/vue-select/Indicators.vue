...
<template>
    <div ref="container" class="indicators-container">
        <button v-if="hasSelectedOption && isClearable && !isLoading" ref="clearButton" type="button"
            class="clear-button" :disabled="isDisabled" @click.stop="emit('clear')">
        ...
        <button v-if="!isLoading" ref="dropdownButton" type="button" class="dropdown-icon"
            :class="{ active: isMenuOpen }" :disabled="isDisabled" @click.stop="emit('toggle')">
